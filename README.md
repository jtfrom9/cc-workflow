...
Claude Code の marketplace 形式で使う場合は、このリポジトリを plugin root として登録する。必要な marketplace metadata は利用環境の Claude Code plugin 管理に合わせて用意する。

## 機能

### 自動 checkpoint

Claude の応答が終わるたびに、現在のコンテキスト使用量を確認する。`Stop` フックでセッショントランスクリプトのトークン消費を見て、同プロジェクトの直前 task より閾値（既定 30%）以上トークンが増えていれば、checkpoint task を `~/.cc-workflow/tasks/<project>/<taskId>/` に作成する。
...
- `summary.md`: 採取時には生成しない。後から `/cc-workflow:summarise` で会話履歴を抜き出して生成する
...
`tools/status_line.py` を Claude Code の `statusLine` に登録すると、現在のコンテキスト使用量を表示できる。
