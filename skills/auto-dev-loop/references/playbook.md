...
**自己言及コメントの禁止**。PR コメント（修正・追加・変更指示）への対応を**同じ PR に push して**
行ったとき、その PR に「同じ PR で対応しました」「この PR で修正しました」式のコメントを返さない。
push した差分はその PR にそのまま現れるので、PR 自身を指す「この PR で対応」は冗長で、しかも別 PR
が存在するかのように読めて紛らわしい。返信する場合は **具体的に何を変えたか**（対象ファイル・
コミット・テスト結果）だけを書き、PR 番号やこの／同じ PR への参照表現は付けない。別 PR・別 issue を
指す参照（`#NNN`「別 PR で対応」）は、対応物が**実際にその別の場所にある**ときに限って使う。
