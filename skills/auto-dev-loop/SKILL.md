...
**自己言及コメントを書かない**: PR のコメントに対して**同じ PR にコミットを push して対応**したとき、
その PR に「同じ PR で対応しました」式の案内コメントを返さない。push した差分はその PR に出るので、
「この PR で対応」は冗長・無意味（むしろ別 PR があるかのように読めて紛らわしい）。返信するなら
**具体的に何を変えたか**（対象ファイル・コミット）だけを述べ、「同じ PR で／この PR で対応」のような
PR への参照表現は付けない。別 PR を指す参照は、対応が**実際に別 PR にある**ときだけ使う。
